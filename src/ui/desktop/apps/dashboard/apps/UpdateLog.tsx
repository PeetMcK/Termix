  ...
  return (
    <>
      ...
      <Sheet open={isOpen} onOpenChange={setIsOpen}>
        <SheetContent
          side="right"
          className="w-[500px] bg-card border-l-2 border-border text-foreground sm:max-w-[500px] p-0 flex flex-col [&>button]:hidden"
        >
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {versionInfo && versionInfo.status === "requires_update" && (
              <Alert className="bg-muted border-border text-foreground mb-3">
            ...
            <div className="space-y-3">
              {releases?.items.map((release) => (
                <div
                  key={release.id}
                  className="border border-border rounded-lg p-3 hover:bg-muted transition-colors cursor-pointer bg-muted/50"
