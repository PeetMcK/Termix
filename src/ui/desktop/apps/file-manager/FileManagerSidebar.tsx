  ...
  const renderSidebarItem = (item: SidebarItem, level: number = 0) => {
    ...
    return (
      <div key={item.id}>
        <div
          className={cn(
            "flex items-center gap-2 py-1.5 text-sm cursor-pointer hover:bg-accent rounded",
      ...
      <div className="h-full flex flex-col bg-background border-r border-border">
        <div className="flex-1 relative overflow-hidden">
          <div className="absolute inset-1.5 overflow-y-auto thin-scrollbar space-y-4">
            ...
            <div
              className={cn(
                hasQuickAccessItems && "pt-4 border-t border-border",
              )}
            >
              <div className="flex items-center gap-2 px-3 py-2 text-xs font-medium text-muted-foreground uppercase tracking-wider">
      ...
      {contextMenu.isVisible && contextMenu.item && (
        <>
          ...
          <div
            ...
            className="fixed bg-background border border-border rounded-lg shadow-xl min-w-[160px] z-50 overflow-hidden"
            style={{
              left: contextMenu.x,
              top: contextMenu.y,
            }}
          >
            {contextMenu.item.type === "recent" && (
              <>
                <button
                  className="w-full px-3 py-2 text-left text-sm flex items-center gap-3 hover:bg-accent text-foreground first:rounded-t-lg last:rounded-b-lg"
                  onClick={() => {
