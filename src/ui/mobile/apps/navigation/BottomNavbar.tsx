...
export function BottomNavbar({ onSidebarOpenClick }: MenuProps) {
  ...
  return (
    <div className="w-full h-[80px] bg-primary-element flex items-center p-2 gap-2">
      <Button
          ...
          {tabs.map((tab) => (
            ...
            >
              <Button
                variant="outline"
                className={cn(
                  "h-10 rounded-r-none !px-3 border-1 border-border",
                  tab.id === currentTab && "!bg-muted-darkest !text-foreground",
                )}
              ...
              <Button
                variant="outline"
                className="h-10 rounded-l-none !px-2 border-1 border-border"
